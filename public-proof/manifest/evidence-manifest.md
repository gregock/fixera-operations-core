...
- do not expose secrets, credentials, client records, deployable private backend code, production databases, or sensitive operational details
...
| Architecture diagram | Private architecture notes, service boundaries, and runtime layout | Redraw as a simplified public architecture view with only stable system components, abstracted artifact categories, and data flows | Omit internal hosts, private networks, deployment secrets, and deployable implementation details | `public-proof/diagrams/` | Cross-check against `docs/architecture.md` and confirm all labels are public-safe |
